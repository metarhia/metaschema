...
class SchemaMetadata extends TypeMetadata {
  ...
  setMany(values) {
    this.kind = values.kind || this.kind;
    this.scope = values.scope || this.scope;
    this.store = values.store || this.store;
    this.allow = values.allow || this.allow;
    this.parent = values.parent || this.parent;
  }
  ...
  validate(value, path) {
    ...
    try {
      return SchemaError.format(this.options.validate(value, path), path);
    } catch (err) {
      return SchemaError.format(`validation failed ${String(err)}`, path);
    }
  }
}
