...
class Model {
  ...
  static async load(modelPath, systemTypes = {}) {
    ...
    const externalTypes = structs.get('.types') || {};
    structs.delete('.database');
    structs.delete('.types');
    const customTypes = Model.loadTypes({ ...systemTypes, ...externalTypes });
    const types = { ...TYPES, ...customTypes };
    return new Model(types, structs, database);
  }

  static loadTypes(customTypes) {
    const types = {};
    for (const [key, value] of Object.entries(customTypes)) {
      if (typeof value === 'string') {
        types[key] = { pg: value, ...TYPES.string };
      } else {
        const { js } = value;
        if (!js) continue;
        const name = js;
        const [, rest] = omit('js', value);
        const exist = TYPES[name];
        types[key] = exist ? { ...exist, ...rest } : { name, ...rest };
      }
    }
    return types;
  }
