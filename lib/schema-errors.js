...
const schemaValidationErrorSerializers = {
  ...
  unresolvedProperty: error => 'Reference to an unresolved property' +
    ` '${error.info.property}' from ${error.source}.${error.property}`,
  duplicateName: error => `Attempt to define field '${error.property}' ` +
    `in ${error.source} which is already defined in ${error.info.location}`,
  unlinkedForm: error => `Form '${error.source}' is not linked to a category`,
};
