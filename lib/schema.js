...
class Schema extends SchemaMetadata {
  ...
  static isFieldEqual(field1, field2) {
    for (const prop of ['type', 'required']) {
  ...
  static concat(...sources) {
    ...
    let kind = schemas[0]?.kind;
    for (const schema of schemas) {
      if (kind !== schema.kind) {
        throw new Error(`Schemas have different kinds: "${kind}", "${schema.kind}"`);
      }
      Object.keys(schema.fields).forEach((key) => fieldKeys.add(key));
    }
    for (const key of Array.from(fieldKeys)) {
      for (const schema of schemas) {
        ...
        if (result[key] === undefined) {
          result[key] = schema.fields[key];
          continue;
        }
        if (!Schema.isFieldEqual(result[key], schema.fields[key])) {
          throw new Error(`Schema concat conflicts with key "${key}"`);
        }
      }
    }
    return Schema.from(result);
  }
