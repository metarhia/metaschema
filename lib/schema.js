...
const checkIterable = (iterable) => (
  validateIterableType,
  validateElementType
) => {
  if (!validateIterableType()) return false;
  for (const element of iterable[Symbol.iterator]()) {
    if (!validateElementType(element)) return false;
  }
  return true;
};

const checks = {
  array: (arr, elementType) =>
    checkIterable(arr)(
      () => Array.isArray(arr),
      (x) => typeof x === elementType
    ),

  set: (set, elementType) =>
    checkIterable(set)(
      () => Reflect.getPrototypeOf(set) === Set.prototype,
      (x) => typeof x === elementType
    ),

  map: (map, keyType, valueType) =>
    checkIterable(map)(
      () => Reflect.getPrototypeOf(map) === Map.prototype,
      ([key, value]) => typeof key === keyType && typeof value === valueType
    ),

  object: (obj, keyType, valueType) =>
    checkIterable(Object.keys(obj))(
      () => typeof obj === 'object',
      (key) => typeof key === keyType && typeof obj[key] === valueType
    ),
};
