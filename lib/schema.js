...
loadSchema('./taxonomy/CategoryField.schema', (error, schema) => {
  categories.set('CategoryField', schema);
});
...
const build = (
  ...
  for (name in schemas) {
    ...
    //for (const fld in definition) {
    //  const val = definition[fld];
    //  const inst = createInstance('CategoryField', val);
    //  console.dir({ val, inst });
    //}
...
const validate = (
  ...
  const { definition } = categories.get(category);
