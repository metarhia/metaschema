...
const isEnumArray = (def) => def && def.array && Array.isArray(def.array.enum);
...
const check = (name, def, val) => {
  ...
  if (type === 'array' || type === 'set') {
    ...
    if (arr.every((v) => def.enum.includes(v))) return [];
...
const shorthand = (def) => {
  ...
  else if (Array.isArray(def.enum) || isEnumArray(def)) return 'enum';
  return '';
};
