...
class Schema {
  ...
  preprocessType(defs) {
    ...
    if (!res) throw new Error('Invalid definition provided');
    const { short, def } = res;
    const isType = short ? this.isType(short) : false;
    if (isType) {
  ...
  projection(metadata) {
    ...
    this.rules = schema;
    const rules = this.findReference(this.rules);
    const defs = {};
    for (const key of fields) {
      defs[key] = rules.fields[key];
    }
    return defs;
  }
  ...
  getTypes() {
    ...
    for (const ns of this.namespaces) {
      names.push(Object.keys(ns.types));
    }
    return names;
  }
