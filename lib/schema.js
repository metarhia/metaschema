...
const validatableEntityTypes = ['domains', 'categories'];

const validationSchemas = {
  domains: 'Domain',
  categories: 'DatabaseField',
};

// Validate schemas
//   schemas - <Iterable> schemas in form [name, schema]
// Returns: <MetaschemaError>
const validateSchemas = schemas => {
  if (!schemaValidator) return null;
  const errors = [];
  for (const entityType of validatableEntityTypes) {
    const entities = schemas[entityType];
    const validationSchema = validationSchemas[entityType];
    for (const { definition } of entities) {
      const err = schemaValidator.validateFields(validationSchema, definition);
      if (err) errors.push(...err.errors);
    }
  }
...
const create = schemas => {
  const err = validateSchemas(schemas);
  if (err) return [err];
