...
const processView = (name, view, ms) => {
  ...
  for (const field of view.Fields) {
    ...
    for (const key in props) {
      ...
      try {
        view.definition[key] = extractByPath(schema.definition, prop, ms, {
          source: name,
          property: `Fields.${key}`,
        });
      } catch (error) {
...
const processForms = ms => {
  ...
  for (const [name, form] of ms.forms) {
    ...
    if (dot === -1) {
      errors.push(new SchemaValidationError('unlinkedForm', name));
      continue;
    }
    ...
    errors.push(
      ...processFields(ms, category.definition, form.Fields, {
        source: name,
        property: 'Fields',
      })
    );
    ...
    for (const [categoryName, { definition: category }] of this.categories) {
      const actions = iter(Object.keys(category))
        ...
        .map(key => {
          ...
          errors.push(
            ...processFields(this, category, def.Args, {
              source: `${categoryName}.${key}`,
              property: 'Args',
            })
          );
          errors.push(
            ...processFields(this, category, def.Returns, {
              source: `${categoryName}.${key}`,
              property: 'Returns',
            })
          );
...
const create = schemas => {
  ...
  const error =
    linkSchemas(schemas, ms.categories, ms.domains) ||
    ms[initActions]() ||
    processViews(ms) ||
    processForms(ms);
