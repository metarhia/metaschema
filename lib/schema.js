...
class Metaschema {
  ...
  // Serialize an instance of schema
  //   type <string>
  //   schema <Schema> | <string> schema or path, that can be resolved by
  //       config.resolve
  //   options <Object>
  // Returns: <string> serialized schema
  serialize(type, schema, options) {
    const processor = this.config.processors[type];
    if (!processor) {
      throw new TypeError(`No processors defined for type: '${type}'`);
    }
    const serializer = processor.serializeSchema;
    if (!serializer) {
      throw new TypeError(`No serializer defined for type: '${type}'`);
    }
    if (typeof schema === 'string') {
      schema = this.config.resolve(this, type, schema);
    }
    return serializer(schema, options);
  }
