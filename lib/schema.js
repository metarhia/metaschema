...
const extractDefinition = (definition, path, ms) => {
  const separatorIndex = path.indexOf('.');
  if (separatorIndex === -1) {
    return definition[path];
  }
  const property = path.slice(0, separatorIndex);
  const categoryName = definition[property].category;
  const category = ms.categories.get(categoryName).definition;
  return extractDefinition(category, path.slice(separatorIndex + 1), ms);
};

const processView = (view, ms) => {
  if (view.definition) {
    return;
  }
  view.definition = {};
...
const processViews = ms => {
  for (const schema of ms.views.values()) {
    processView(schema, ms);
  }
};

const processFields = (ms, category, fields) => {
  for (const key in fields) {
    const arg = fields[key];
    Object.assign(arg, arg.domain ?
      { domain: arg.domain, definition: ms.domains.get(arg.domain) } :
...
class Metaschema {
  ...
  validateForm(formName, args) {
    ...
    const errors = this[validate](form.Args, args, false, `${formName}.`);
      ...
      const actions = iter(Object.keys(category))
        ...
        .map(key => {
          ...
          const action = {
            ...
            form: this.forms.get(`${categoryName}.${def.Form || key}`),
          };
          return [key, action];
        }).collectTo(Map);
