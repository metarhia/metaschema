...
class Schema {
  ...
  preprocess(defs) {
    ...
    for (const key of keys) {
      ...
      const collection = detectType(def);
      if (Array.isArray(def) && key === 'enum') {
        delete this.fields.enum;
        this.fields.value = defs;
        return;
      } else if (collection) {
        const colType = def[collection];
        delete def[collection];
        if (collection === 'array' || collection === 'set') {
          Object.assign(def, { type: collection, value: colType });
        } else if (collection === 'object' || collection === 'map') {
          const key = Object.keys(colType)[0];
          const value = colType[key];
          Object.assign(def, { type: collection, key, value });
        }
      } else {
