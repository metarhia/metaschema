...
//   value - <any>
//   path - <string>
//   definition - <Object>, link definition
//   options - <Object>
//     patch - <boolean>, flag to determine if the object contains patch or
//         value, default: `false`
//     allowAdditionalFields - <boolean>, flag to determine if fields that are
//         not defined in schema should be allowed, default: `true`
// Returns: <ValidationError[]>
const validateLink = (
  value, path, definition, ms, { patch, allowAdditionalFields }
) => {
  const errors = [];
  ...
  if (type === 'Include') {
    return ms[validate](
      category, value, { patch, path: `${path}.`, allowAdditionalFields }
    );
  }
...
class Metaschema {
  ...
  // Returns: <ValidationError[]>
  [validate](
    schema, object, { patch = false, path = '', allowAdditionalFields = true }
  ) {
    const errors = [];
  ...
  //         not defined in schema should be allowed, default: `true`
  // Returns: <MetaschemaError> | <null>
  validateCategory(categoryName, instance, options = {}) {
    ...
    options.path = options.path !== undefined ? options.path + suffix : suffix;

    const errors = this[validate](category.definition, instance, options);
  ...
  //         not defined in schema should be allowed, default: `true`
  // Returns: <MetaschemaError> | <null>
  validateForm(formName, args, options = {}) {
    ...
    options.patch = false;
    options.path = `${formName}.`;
    const errors = this[validate](form, args, options);
  ...
  //         not defined in schema should be allowed, default: `true`
  // Returns: <MetaschemaError> | <null>
  validateFields(category, instance, options = {}) {
    const errors = [];
    for (const key in instance) {
      const val = instance[key];
      const opts = Object.assign({}, options, { path: `${key}.` });
      const err = this.validateCategory(category, val, opts);
